...
C takes a calendar date string as mm/dd/yyyy and returns the month, day and year
      ...
      return
       ...
       Integer Function GetHour(DateTime)
c      reads a date value with a time and returns the hour
       character*20 DateTime, Temp
       character*10 cDate, cHour
       character*10 cTime
       integer pos,Length, GHour

       ! first split date from time by finding :
       Length=len(DateTime)
       pos=index(DateTime,':')
       cDate=DateTime(1:pos-1)
       cHour=DateTime(pos+1:pos+2)
       read(cHour,*) GHour  
       Gethour=GHour
